...
卒業論文第5章では、直交多項式の近似理論を考える準備としてワイエルシュトラスの近似定理を扱っている。証明にはベルンシュタイン多項式を用いる。
...
卒論では、まず $f(x)=1,x,x^2$ に対する計算を行い、その後一般の連続関数へ拡張する。
...
この定理は、連続関数を多項式で近似できることを保証する。卒論では、この結果を直交多項式による近似や $L^2$ 空間における完備性の議論へつなげている。
