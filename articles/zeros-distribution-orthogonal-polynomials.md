...
直交多項式の零点には、隣り合う次数だけでなく、離れた次数の間にも規則性がある。卒業論文第8章では、ガウスの求積公式を利用して零点の分布をさらに詳しく調べている。
...
$(\alpha,\beta)$ の内部では正、外部では負になるような連続関数を考える。卒論では、十分小さい $\varepsilon>0$ に対して、
...
卒論では、この零点の稠密化に関する結果は $(a,b)$ が無限区間の場合にはそのまま成立しないことにも注意している。有限区間という仮定が重要である。
