...
卒業論文第7章では、正規直交多項式を用いて関数を多項式で近似するとき、どの多項式が $L^2$ の意味で最良近似になるかを考えている。本記事では、その中心となる結果をまとめる。
...
卒論では、連続関数 $f$ と最良近似多項式 $p_n$ に対して、$f-p_n$ の符号変化についても考察している。本文では、$f-p_n$ が恒等的に $0$ でない場合には区間 $(a,b)$ 内で少なくとも $n-1$ 回符号を変えるという性質を扱っている。
